...
bool network_rule::gen_ip_conds(Profile &prof, std::list<std::ostringstream> &streams, ip_conds &entry,
				bool is_peer, bool is_cmd, bool is_iface)
{
	std::string buf;
	perm32_t cond_perms;
	...
	if ((!is_cmd && (entry.label_match || is_peer)) || is_iface) {
		...
		cond_rule_mode = rule_mode_t::UNSPECIFIED;
	}
	...
	for (auto &oss : streams) {
		...
		if (is_iface) {
			...
			perm32_t if_perms = cond_perms;
			rule_mode_t if_rule_mode = cond_rule_mode;
			if (!entry.label_match && !is_peer) {
				if_perms = map_perms(perms);
				if_rule_mode = rule_mode;
			}
			if (perms & AA_MAY_SEND)
				if_perms |= AA_SET_LABEL;
			if (!prof.policy.rules->add_rule(buf.c_str(), priority,
							 if_rule_mode, if_perms,
							 dedup_perms_rule_t::audit == audit_t::FORCE ? map_perms(perms) : 0,
							 parseopts))
				return false;
		}
...
bool network_rule::gen_net_rule(Profile &prof, unsigned int netclass, u16 family, unsigned int type_mask, unsigned int protocol, bool skb) {
	...
	if (!prof.policy.rules->add_rule(buf.c_str(), priority,
					 rule_mode_t::UNSPECIFIED, AA_COMPAT_CONT_MATCH, 0,
					 parseopts))
		return false;
	...
	if (perms & AA_PEER_NET_PERMS) {
		...
		if (!gen_ip_conds(prof, streams, peer, true, false, is_iface))
			return false;
		...
		if (!gen_ip_conds(prof, streams, local, false, true, is_iface))
			return false;
	}
	...
	if (!gen_ip_conds(prof, streams, local, false, false, false))
		return false;
