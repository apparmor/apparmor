...
= does a straight assignment of the right hand side to the variable. The
  variable must not be declared before or an error will occur. If there
  is a chance that the variable could be declared before the += or ?=
  assignment operators can be used instead.
